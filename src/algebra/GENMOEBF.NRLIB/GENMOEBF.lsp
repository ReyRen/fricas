...
(SDEFUN |GENMOEBF;generalizedMoebiusFunction;LM$;4|
        ...
        (SPROG
         ((#1=#:G400 NIL) (|mf| (|Union| (|Matrix| R) "failed"))
          (|zf| (|Matrix| R)) (#2=#:G404 NIL) (|x| NIL) (#3=#:G403 NIL)
          (#4=#:G402 NIL) (|y| NIL) (#5=#:G401 NIL) (|xxo| (|List| P)))
         (SEQ
          (EXIT
           (SEQ
            (LETT |xxo|
                 ...
                 (SEQ
                  (COND
                   ((SPADCALL |pi| |si| (QREFELT $ 12))
                    (PROGN (LETT #1# (|spadConstant| $ 25)) (GO #2=#:G406))))
                  (EXIT (|spadConstant| $ 17))))
                #2# (EXIT #1#)))) 

(SDEFUN |GENMOEBF;apply;$2PR;6| ((|mf| $) (|x| P) (|y| P) ($ R))
        (SPROG
         ((#1=#:G413 NIL) (|ky| #2=(|Integer|)) (|kx| #2#)
          (|mfn| (|Matrix| R)))
         (SEQ
          (EXIT
...
(DEFUN |GeneralizedFiniteMoebiusFunction| (&REST #1=#:G419)
  (SPROG NIL
         (PROG (#2=#:G420)
           (RETURN
            (COND
