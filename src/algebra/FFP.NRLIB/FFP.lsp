...
(SDEFUN |FFP;basis;PiV;3| ((|n| |PositiveInteger|) ($ |Vector| $))
        (SPROG ((#1=#:G396 NIL) (|i| NIL) (#2=#:G395 NIL) (|a| ($)))
               (SEQ
                (COND
...
(SDEFUN |FFP;degree;$Pi;4| ((|x| $) ($ |PositiveInteger|))
        (SPROG
         ((#1=#:G399 NIL) (|y| ($)) (#2=#:G402 NIL) (|i| NIL)
          (|m| (|Matrix| GF)))
         (SEQ (LETT |y| (|spadConstant| $ 31))
              (LETT |m|
              ...
              (SEQ (LETT |i| 1) (LETT #2# (+ (QREFELT $ 12) 1)) G190
                   ...
                   (SEQ
                    (SPADCALL |m| |i| (SPADCALL |y| (QREFELT $ 52))
                              (QREFELT $ 54))
                    (EXIT (LETT |y| (SPADCALL |y| |x| (QREFELT $ 55)))))
                   (LETT |i| (|inc_SI| |i|)) (GO G190) G191 (EXIT NIL))
              (EXIT
               (PROG1 (LETT #1# (SPADCALL |m| (QREFELT $ 57)))
...
(SDEFUN |FFP;minimalPolynomial;$Sup;5|
        ...
        (SPROG
         ((#1=#:G406 NIL) (#2=#:G405 #3=(|SparseUnivariatePolynomial| GF))
          (#4=#:G407 #3#) (#5=#:G411 NIL) (|i| NIL) (|v| (|Vector| GF))
          (|y| ($)) (#6=#:G410 NIL) (|m| (|Matrix| GF)))
         (SEQ (LETT |y| (|spadConstant| $ 31))
              (LETT |m|
                    (SPADCALL (QREFELT $ 12) (+ (QREFELT $ 12) 1)
                              (QREFELT $ 50)))
              (SEQ (LETT |i| 1) (LETT #6# (+ (QREFELT $ 12) 1)) G190
                   ...
                   (SEQ
                    ...
                    (EXIT (LETT |y| (SPADCALL |y| |x| (QREFELT $ 55)))))
                   (LETT |i| (|inc_SI| |i|)) (GO G190) G191 (EXIT NIL))
              (LETT |v| (|SPADfirst| (SPADCALL |m| (QREFELT $ 60))))
              (EXIT
               (PROGN
                (LETT #1# NIL)
                (SEQ (LETT |i| 0) (LETT #5# (QREFELT $ 12)) G190
                     ...
                     (SEQ
                      (EXIT
                       (PROGN
                        (LETT #4#
                              (SPADCALL (SPADCALL |v| (+ |i| 1) (QREFELT $ 61))
                                        |i| (QREFELT $ 33)))
                        (COND
                         (#1# (LETT #2# (SPADCALL #2# #4# (QREFELT $ 62))))
                         ('T (PROGN (LETT #2# #4#) (LETT #1# 'T)))))))
                     (LETT |i| (|inc_SI| |i|)) (GO G190) G191 (EXIT NIL))
                (COND (#1# #2#) ('T (|spadConstant| $ 63)))))))) 

(SDEFUN |FFP;normal?;$B;6| ((|x| $) ($ |Boolean|))
        (SPROG ((|l| (|List| (|List| GF))) (|a| ($)) (#1=#:G416 NIL) (|i| NIL))
               (SEQ
                (LETT |l|
                      (LIST
                ...
                (SEQ (LETT |i| 2) (LETT #1# (QREFELT $ 12)) G190
                     (COND ((|greater_SI| |i| #1#) (GO G191)))
                     (SEQ (LETT |a| (SPADCALL |a| (QREFELT $ 67)))
                          (EXIT
                           (LETT |l|
                                 (SPADCALL |l|
                                           (SPADCALL
...
(SDEFUN |FFP;initializeLog| (($ |Void|))
        (SPROG
         ((#1=#:G480 NIL) (|a| ($)) (#2=#:G486 NIL) (#3=#:G474 NIL) (|i| NIL)
          (|tbl| (|Table| (|PositiveInteger|) (|NonNegativeInteger|)))
          (|n| (|Integer|)) (|d| (|Integer|)) (|l| (|Integer|)) (|base| ($))
          (|fac| (|Integer|)) (#4=#:G485 NIL) (|f| NIL) (|limit| (|Integer|)))
         (SEQ (COND ((QREFELT $ 26) (|FFP;initializeElt| $))) (LETT |limit| 30)
              (SEQ (LETT |f| NIL) (LETT #4# (QREFELT $ 22)) G190
                   (COND
...
(DEFUN |FiniteFieldExtensionByPolynomial| (&REST #1=#:G524)
  (SPROG NIL
         (PROG (#2=#:G525)
           (RETURN
            (COND
...
(DEFUN |FiniteFieldExtensionByPolynomial;| (|#1| |#2|)
  (SPROG
   ((|pv$| NIL) (#1=#:G523 NIL) ($ NIL) (|dv$| NIL) (DV$2 NIL) (DV$1 NIL))
   (PROGN
            ...
            (CONS
             ...
             (CONS
              '#((|FiniteAlgebraicExtensionField| 6) (|ExtensionField| 6)
                 ...
                 (|BasicType|) (|CoercibleTo| 15) (|ConvertibleTo| 150))
